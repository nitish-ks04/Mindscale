...
export default function Chatbot() {
  ...
  const sendMessage = async () => {
    ...
    const userMsg: IChatMessage = { sender: "user", text: input };
    ...
    try {
      ...
      console.error(err);
      setIsThinking(false);
      const errMsg: IChatMessage = { sender: "bot", text: "⚠️ Error connecting to chatbot." };
  ...
  const handleSaveChat = async (index: number) => {
    try {
      const token = localStorage.getItem("userInside");
      if (!token) {
              ...
              {messages.length === 0 ? (
                ...
                <>
                  {messages.map((msg, i) => (
                    <div
                      key={i}
                      className={`flex mb-6 animate-fadeIn ${msg.sender === "user" ? "justify-end" : "justify-start"
                        }`}
                    >
                      {msg.sender === "bot" && (
                      ...
                      <div
                        className={`max-w-[75%] break-words overflow-hidden ${msg.sender === "user"
                        ...
                        {msg.sender === "bot" && (
                          <div className="flex justify-between items-center mb-3 pb-3 border-b border-slate-700">
                            <div className="flex items-center gap-2">
                              ...
                              {msg.sentiment && (
                                <span
                                  className={`text-xs font-medium px-2.5 py-1 rounded-full ${msg.sentiment.emotion === "positive"
