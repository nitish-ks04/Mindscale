...
function History() {
  ...
  const handleDelete = async (id: string) => {
    try {
      ...
      setItems((prev: IHistoryItem[]) => prev.filter((chat: IHistoryItem) => chat._id !== id));
  ...
  return (
    <>
      <Navbar
        onToggleSidebar={() => setSidebarOpen((s: boolean) => !s)}
        isSidebarOpen={sidebarOpen}
      />
      <Sidebar isOpen={sidebarOpen} />

      <div
        className={`min-h-screen bg-gradient-to-br from-slate-950 via-slate-900 to-slate-950 pt-24 pb-10 px-4 flex justify-center transition-all duration-300 ${sidebarOpen ? "pl-60" : "pl-4"
          }`}
      >
